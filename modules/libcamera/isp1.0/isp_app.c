	...
	if (ISP_APP_ZERO != (ISP_CALLBACK_EVT&mode)) {
		if ((ISP_AF_NOTICE_CALLBACK == (ISP_EVT_MASK&mode))
			...
			if((ISP_AE_BAPASS_CALLBACK == (ISP_EVT_MASK&mode))
				...
				isp_context_ptr->ctrl_callback((unsigned long)isp_context_ptr->oem_handle, ISP_CALLBACK_EVT|ISP_AE_STAB_CALLBACK, param_ptr, param_len);

			} else {
				isp_context_ptr->ctrl_callback((unsigned long)isp_context_ptr->oem_handle, mode, param_ptr, param_len);
			}
		}
	} else {
		//isp_context_ptr->ctrl_callback(handler_id, mode, param_ptr, param_len);
...
uint32_t _isp_AppStopVideoHandler(uint32_t handler_id)
{
	...
	if (ISP_APP_EB == isp_context_ptr->af_flag) {
		...
		isp_context_ptr->ctrl_callback((unsigned long)isp_context_ptr->oem_handle,ISP_CALLBACK_EVT|ISP_AF_NOTICE_CALLBACK, (void*)&af_notice, sizeof(struct isp_af_notice));
