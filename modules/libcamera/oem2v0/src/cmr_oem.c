	                                               ...
	                                               cmr_uint is_snapshot, struct preview_param *out_param_ptr)
{
	...
	cxt->snp_cxt.total_num = setting_param.cmd_type_value;
        ...
        int encode_angle = jpeg_cxt->param.set_encode_rotation;
	int flip_on = setting_param.cmd_type_value;
	if(encode_angle == IMG_ANGLE_90 || encode_angle == IMG_ANGLE_270){
		out_param_ptr->flip_on = flip_on;       // when screen rotate 0 or 180, path2 do horizontal mirror
	} else {
		if(flip_on == 0){
			out_param_ptr->flip_on = flip_on; //no mirror
		} else {
			out_param_ptr->flip_on = DCAM_PATH_VERTICAL_MIRROR;	// when screen rotate 90 or 270, path2 do vertical mirror
		}
	}
...
cmr_int camera_local_highflash_ae_measure (cmr_handle oem_handle)
{
	...
	if ((CAMERA_ZSL_MODE != cxt->snp_cxt.snp_mode)
		&& (CAMERA_ISP_TUNING_MODE != cxt->snp_cxt.snp_mode)
