...
static unsigned long _s5k4h5yc_PowerOn(unsigned long power_on)
{
	...
	SENSOR_PRINT_ERR("SENSOR_S5K4H5YC: _s5k4h5yc_Power_On(1:on, 0:off): %d, reset_level %d, dvdd_val %d", power_on, reset_level, dvdd_val);
...
//				*p=&calibration_info;
		...
		case SENSOR_VAL_TYPE_GET_GOLDEN_LSC_DATA:
			*(uint32_t*)param_ptr->pval = *(uint32_t*)lsc_golden_data;
			break;
		default:
			break;
	}
