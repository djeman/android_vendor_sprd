...
struct setting_component {
	...
	cmr_int                       force_set;
	sem_t                         flash_ready_sem;
	sem_t                         flash_stab_sem;
...
static cmr_int cmr_setting_init_sem (struct setting_component *cpt);
static cmr_int setting_isp_wait_flash_ready (struct setting_component *cpt);
static cmr_int setting_isp_wait_flash_stab (struct setting_component *cpt);
static cmr_int setting_isp_wait_flash_stab_withtime (struct setting_component *cpt, cmr_uint timeout);
...
static cmr_int setting_isp_flash_handle(struct setting_component *cpt,
						struct setting_cmd_parameter *parm,
						  cmr_uint flash_mode)
{
	cmr_uint                 status = FLASH_STATUS_MAX;
				                      ...
				                      struct setting_cmd_parameter *parm)
{
	...
	flash_mode = parm->cmd_type_value;
	flash_param->flash_mode = flash_mode;
	setting_isp_flash_handle(cpt, parm, flash_param->flash_mode);
				                   ...
				                   struct setting_cmd_parameter *parm)
{
	...
	if (is_changed) {
                /*update zoom unit before processed or not*/
                pthread_mutex_lock(&cpt->status_lock);
				                     ...
				                     struct setting_cmd_parameter *parm)
{
	...
	if (NULL != p_exif_info->primary.data_struct_ptr) {
		...
		//for bug559854
		#ifdef CONFIG_SPRD_PRIVATE_ZSL
		if(hal_param->sprd_zsl_enabled && parm->camera_id == 1 && hal_param->flip_on == 1){
			if(hal_param->encode_angle == IMG_ANGLE_90 || hal_param->encode_angle == IMG_ANGLE_270){
				p_exif_info->primary.data_struct_ptr->Orientation = ORIENTATION_ROTATE_180;
			}
		}
		#endif
	}
					              ...
					              struct setting_cmd_parameter *parm)
{
	...
	CMR_LOGI("is_active %ld, flash_mode %ld, ctrl_flash_status %d flash_hw_status %ld",
		is_active, flash_mode, (cmr_u32)ctrl_flash_status, flash_hw_status);

	work_mode = parm->ctrl_flash.work_mode;
	if (is_active) {
		if (flash_hw_status == FLASH_OPEN) {//continue touch screen,just do ae quick measure
			cmr_setting_init_sem(cpt);
			setting_isp_flash_notify(cpt, parm, ISP_FLASH_PRE_LIGHTING);
			setting_isp_wait_flash_stab(cpt);
			goto EXIT;
		}
		if ((CAMERA_FLASH_MODE_AUTO == flash_mode)
			&& ((ctrl_flash_status == FLASH_OPEN) || (flash_hw_status == FLASH_CLOSE))) {
		...
		if (setting_is_need_flash(cpt, parm)) {
			if (IMG_DATA_TYPE_RAW == image_format) {
				if(ctrl_flash_status == FLASH_HIGH_LIGHT) { //high flash
					...
					setting_isp_wait_flash_ready(cpt);
...
static cmr_int cmr_setting_init_sem (struct setting_component *cpt)
{
	...
	sem_init(&cpt->flash_ready_sem, 0, 0);
	sem_init(&cpt->flash_stab_sem,0,0);
...
static cmr_int setting_isp_wait_flash_ready (struct setting_component *cpt)
{
	...
	if (clock_gettime(CLOCK_REALTIME, &ts)) {
		...
		pthread_mutex_unlock(&cpt->isp_mutex);
		if (cmr_sem_timedwait((&cpt->flash_ready_sem), &ts)) {
			pthread_mutex_lock(&cpt->isp_mutex);
			rtn = -1;
			CMR_LOGW("timeout.");
		} else {
...
static cmr_int setting_isp_wait_flash_stab (struct setting_component *cpt)
{
	return setting_isp_wait_flash_stab_withtime(cpt, ISP_ALG_TIMEOUT);
}
...
static cmr_int setting_isp_wait_flash_stab_withtime (struct setting_component *cpt, cmr_uint timeout)
{
	...
	if (clock_gettime(CLOCK_REALTIME, &ts)) {
		...
		ts.tv_sec += timeout;
		pthread_mutex_unlock(&cpt->isp_mutex);
		if (cmr_sem_timedwait((&cpt->flash_stab_sem), &ts)) {
			pthread_mutex_lock(&cpt->isp_mutex);
			rtn = -1;
			CMR_LOGW("timeout.");
		} else {
...
cmr_int cmr_setting_flash_ready (cmr_handle setting_handle, void *data)
{
	...
	pthread_mutex_lock(&cpt->isp_mutex);
	CMR_LOGI("release flash_ready.");
	cmr_sem_post(&cpt->flash_ready_sem);
	pthread_mutex_unlock(&cpt->isp_mutex);
	return 0;
}

cmr_int cmr_setting_flash_stab (cmr_handle setting_handle, void *data)
{
	struct setting_component *cpt = (struct setting_component *)setting_handle;
	UNUSED(data);
	...
	pthread_mutex_lock(&cpt->isp_mutex);
	CMR_LOGI("release flash_stab.");
	cmr_sem_post(&cpt->flash_stab_sem);
	pthread_mutex_unlock(&cpt->isp_mutex);
	return 0;
}
	...
	if (!been_preflash) {
		...
		if (setting_is_need_flash(cpt, parm)) {
			...
			if (IMG_DATA_TYPE_RAW == image_format) {
				...
				setting_isp_flash_notify(cpt, parm, ISP_FLASH_PRE_LIGHTING);
				setting_isp_wait_flash_stab_withtime(cpt, ISP_PREFLASH_ALG_TIMEOUT);
			} else {
				setting_set_flashdevice(cpt, parm, (uint32_t)FLASH_OPEN);
			}
		}
...
cmr_int cmr_setting_deinit_notice(cmr_handle setting_handle){
	...
	pthread_mutex_unlock(&cpt->isp_mutex);
	sem_post(&cpt->flash_ready_sem); // fastly quit af process when flash on,--for safty quit post two times
	sem_post(&cpt->flash_stab_sem);
...
cmr_int cmr_setting_deinit(cmr_handle setting_handle)
{
	...
	sem_destroy(&cpt->flash_ready_sem);
	sem_destroy(&cpt->flash_stab_sem);
