...
static int alloc_device_free(alloc_device_t *dev, buffer_handle_t handle)
{
	...
	if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
		...
		if (ion_free(m->ion_client, hnd->ion_hnd) != 0)
...
int alloc_device_open(hw_module_t const *module, const char *name, hw_device_t **device)
{
	...
	dev->common.module = const_cast<hw_module_t*>(&m->base.common);
	dev->common.close = alloc_device_close;
	dev->alloc = alloc_device_alloc;
	dev->free = alloc_device_free;

#if GRALLOC_ARM_DMA_BUF_MODULE
	private_module_t *mm = reinterpret_cast<private_module_t *>(dev->common.module);
	mm->ion_client = ion_open();
	if (mm->ion_client < 0)	{
