  ...
  for (i = 0; i < mActiveContextCount; i++) {
    ...
    if (i == DISPLAY_PRIMARY) {
      ...
      for (j = 0; j < ctx->LayerCount; j++) {
        ...
        if (ADP_USAGE(privateH) & GRALLOC_USAGE_HW_FB) {
          mFBTDisplay = true;
          mFBTLayer = l;
          ret = 0;
          continue;
  ...
  for (unsigned int i = 0; i < NUM_DISPLAY_ATTRIBUTES - 1; i++) {
    switch (attributes[i]) {
      ...
      case HWC_DISPLAY_DPI_X:
        values[i] = (int32_t)(mFBDInfo->xdpi*1000);
        break;
      case HWC_DISPLAY_DPI_Y:
        values[i] = (int32_t)(mFBDInfo->ydpi*1000);
        break;
      default:
...
int SprdFrameBufferDevice::configPrimaryPlane(FlushContext_t *ctx,
  ...
  MemoryHeapIon::Get_phy_addr_from_ion(ADP_BUFFD(privateH), &(ADP_PHYADDR(privateH)),
                                       &size);

  ctx->base.buffer = (unsigned char *)(ADP_PHYADDR(privateH));
  ...
  if (mFBDInfo->fd < 0) {
    ALOGE("SprdFrameBufferDevice:: configPrimaryPlane mFBD fd: %d < 0", mFBDInfo->fd);
    return -1;
  }
...
int SprdFrameBufferDevice::configOverlayPlane(FlushContext_t *ctx,
  ...
  if (mFBDInfo->fd < 0) {
    ALOGE("SprdFrameBufferDevice:: configOverlayPlane mFBDeviceFd: %d < 0", mFBDInfo->fd);
    return -1;
  }
...
int SprdFrameBufferDevice::frameBufferDevicePost() {
  ...
  if (mFBTDisplay) {
    ...
    native_handle *privateH = mFBTLayer->getBufferHandle();
