/**
  ...
  $Date: 2008-08-08 07:49:34 +0200 (Fri, 08 Aug 2008) $
  Revision $Rev: 582 $
...
#define COMPONENT_NAME_BASE "OMX.vpu.video_decoder"
#define BASE_ROLE "video_decoder.avc"
#define COMPONENT_NAME_BASE_LEN 21
...
int main(int argc1, char** argv1) {
  ...
  if(flagIsColorConvRequested == 1) {
    ...
    if(flagIsSinkRequested == 1) {
      err = OMX_GetHandle(&appPriv->fbdev_sink_handle, "OMX.st.video.xvideo_sink" /*OMX.st.fbdev.fbdev_sink"*/, NULL, &fbdev_sink_callbacks);
      if(err != OMX_ErrorNone){
