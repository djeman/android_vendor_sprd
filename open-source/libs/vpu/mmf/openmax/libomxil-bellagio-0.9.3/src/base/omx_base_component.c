...
do { \
    if (n == DEB_LEV_ERR) \
    { \
        ALOGE("[TID:%d-%s-%d]OMX- " fmt, gettid(), __func__, __LINE__, ##args); \
    } \
    else if (DEBUG_LEVEL & (n)) \
    { \
        ALOGI("[TID:%d-%s-%d]OMX- " fmt, gettid(), __func__, __LINE__, ##args); \
    } \
} while (0)
#endif
...
void base_constructor_remove_garbage_collected(omx_base_component_PrivateType* omx_base_component_Private) {
    ...
    if (omx_base_component_Private->deinterlace_flush_condition) {
        tsem_deinit(omx_base_component_Private->deinterlace_flush_condition);
        free(omx_base_component_Private->deinterlace_flush_condition);
    ...
    if (omx_base_component_Private->bellagioThreads) {
        ...
        omx_base_component_Private->bellagioThreads = NULL;
    }
    ...
    if (omx_base_component_Private->bDispBufFullSem) {
        tsem_deinit(omx_base_component_Private->bDispBufFullSem);
        free(omx_base_component_Private->bDispBufFullSem);
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
    ...
    if (openmaxStandComp->pComponentPrivate) {
        ...
        omx_base_component_Private = malloc(sizeof(omx_base_component_PrivateType));
        if (!omx_base_component_Private) {
    ...
    if(!omx_base_component_Private->messageQueue) {
        omx_base_component_Private->messageQueue = malloc(sizeof(queue_t));
        if (!omx_base_component_Private->messageQueue) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
    ...
    if(!omx_base_component_Private->messageSem) {
        omx_base_component_Private->messageSem = malloc(sizeof(tsem_t));
        if (!omx_base_component_Private->messageSem) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->messageSem, 0x00, sizeof(tsem_t));
    ...
    if(!omx_base_component_Private->bMgmtSem) {
        omx_base_component_Private->bMgmtSem = malloc(sizeof(tsem_t));
        if (!omx_base_component_Private->bMgmtSem) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->bMgmtSem, 0x00, sizeof(tsem_t));
...
#ifdef WAITING_TIME_FOR_NEXT_OUTPUT_BUFFER_AFTER_DISPLAY_BUFFER_FULL
    if(!omx_base_component_Private->bDispBufFullSem) {
        omx_base_component_Private->bDispBufFullSem = malloc(sizeof(tsem_t));
        if (!omx_base_component_Private->bDispBufFullSem) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->bDispBufFullSem, 0x00, sizeof(tsem_t));

        err = tsem_init(omx_base_component_Private->bDispBufFullSem, 0);
        if (err != 0) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
...
#endif
    if(!omx_base_component_Private->bStateSem) {
        omx_base_component_Private->bStateSem = malloc(sizeof(tsem_t));
        if (!omx_base_component_Private->bStateSem) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->bStateSem, 0x00, sizeof(tsem_t));
    ...
    omx_base_component_Private->name = malloc(OMX_MAX_STRINGNAME_SIZE);
    if (!omx_base_component_Private->name) {
        base_constructor_remove_garbage_collected(omx_base_component_Private);
        return OMX_ErrorInsufficientResources;
    }
    memset(omx_base_component_Private->name, 0x00, OMX_MAX_STRINGNAME_SIZE);
    ...
    omx_base_component_Private->bellagioThreads = malloc(sizeof(OMX_PARAM_BELLAGIOTHREADS_ID));
    if (omx_base_component_Private->bellagioThreads == NULL) {
        base_constructor_remove_garbage_collected(omx_base_component_Private);
        return OMX_ErrorInsufficientResources;
    }
    memset(omx_base_component_Private->bellagioThreads, 0x00, sizeof(OMX_PARAM_BELLAGIOTHREADS_ID));
    ...
    if(!omx_base_component_Private->flush_all_condition) {
        omx_base_component_Private->flush_all_condition = malloc(sizeof(tsem_t));
        if(!omx_base_component_Private->flush_all_condition) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->flush_all_condition, 0x00, sizeof(tsem_t));
    ...
    if(!omx_base_component_Private->flush_condition) {
        omx_base_component_Private->flush_condition = malloc(sizeof(tsem_t));
        if(!omx_base_component_Private->flush_condition) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->flush_condition, 0x00, sizeof(tsem_t));
    ...
    if(!omx_base_component_Private->deinterlace_flush_condition) {
        omx_base_component_Private->deinterlace_flush_condition = malloc(sizeof(tsem_t));
        if(!omx_base_component_Private->deinterlace_flush_condition) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
        memset(omx_base_component_Private->deinterlace_flush_condition, 0x00, sizeof(tsem_t));

        err = tsem_init(omx_base_component_Private->deinterlace_flush_condition, 0);
        if (err != 0) {
            base_constructor_remove_garbage_collected(omx_base_component_Private);
            return OMX_ErrorInsufficientResources;
        }
...
#ifdef ANDROID
    pthread_join(omx_base_component_Private->messageHandlerThread, (void **)&err);
    if(err!=0) {
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_DoStateSet(OMX_COMPONENTTYPE *openmaxStandComp, OMX_U32 destinationState) {
    ...
    if(destinationState == OMX_StateLoaded) {
        ...
        DEBUG(DEB_LEV_FUNCTION_NAME, "Out of %s for component %p with err %x\n", __func__, openmaxStandComp, err);
        return err;
    }
    ...
    if(destinationState == OMX_StateIdle) {
        ...
        case OMX_StateLoaded:
            ...
            if (bExit) {
                DEBUG(DEB_LEV_FULL_SEQ, "bExit = true!! break!!\n");
                break;
            }
            DEBUG(DEB_LEV_FULL_SEQ, "bExit = false!! continue!!\n");
            omx_base_component_Private->state = OMX_StateIdle;
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_GetComponentVersion(OMX_HANDLETYPE hComponent,
    ...
    uuid[0] = (unsigned long)omx_component;
    uuid[1] = getpid();
    uuid[2] = getuid();
...
OSCL_EXPORT_REF OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_GetParameter(
    ...
    DEBUG(DEB_LEV_FUNCTION_NAME, "In %s for component %p\n", __func__, hComponent);
    DEBUG(DEB_LEV_FULL_SEQ, "Getting parameter %i\n", nParamIndex);
        ...
        }
        {
            ...
            if ((omx_base_component_Private->state == OMX_StateIdle ||
                ...
                if(pPort->pInternalBufferStorage) {
                    ...
                    pPort->pInternalBufferStorage = malloc(pPort->sPortParam.nBufferCountActual*sizeof(OMX_BUFFERHEADERTYPE *));
                    if (!pPort->pInternalBufferStorage) {
                    ...
                    DEBUG(DEB_LEV_FULL_SEQ, "In %s realloc pInternalBufferStorage for malloc nBufferCountActual=%d\n", __func__, (int)pPort->sPortParam.nBufferCountActual);
                }

                if(pPort->bBufferStateAllocated) {
                    free(pPort->bBufferStateAllocated);
                    pPort->bBufferStateAllocated = NULL;
                    pPort->bBufferStateAllocated = malloc(pPort->sPortParam.nBufferCountActual*sizeof(BUFFER_STATUS_FLAG));
                    if (!pPort->bBufferStateAllocated) {
                        DEBUG(DEB_LEV_ERR, "Insufficient memory in %s\n", __func__);
                        err = OMX_ErrorInsufficientResources;
                        break;
                    ...
                    DEBUG(DEB_LEV_FULL_SEQ, "In %s realloc bBufferStateAllocated for malloc nBufferCountActual=%d\n", __func__, (int)pPort->sPortParam.nBufferCountActual);
                }
            }
        }
        break;
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_SendCommand(
    ...
    message = malloc(sizeof(internalRequestMessageType));
    if (!message) {
        DEBUG(DEB_LEV_ERR, "Insufficient memory in %s\n", __func__);
        return OMX_ErrorInsufficientResources;
    }
...
void* compMessageHandlerFunction(void* param) {
    ...
    while(1) {
        ...
        DEBUG(DEB_LEV_SIMPLE_SEQ, "omx_base_component_Private->messageSem->num : %d",omx_base_component_Private->messageSem->semval);
        ...
        /* Dequeue it */
        DEBUG(DEB_LEV_FUNCTION_NAME, "/* Dequeue it */\n");
        ...
        DEBUG(DEB_LEV_FUNCTION_NAME, "omx_base_component_Private->messageHandler(openmaxStandComp, message)\n");
        ...
        */
        DEBUG(DEB_LEV_FUNCTION_NAME, "free Message\n");
        if (message) {
            free(message);
            message = NULL;
        }
...
OSCL_EXPORT_REF OMX_ERRORTYPE omx_base_component_FillThisBuffer(
    ...
    DEBUG(DEB_LEV_FUNCTION_NAME, "In %s for component %p pBuffer = %p\n", __func__, hComponent, pBuffer);
