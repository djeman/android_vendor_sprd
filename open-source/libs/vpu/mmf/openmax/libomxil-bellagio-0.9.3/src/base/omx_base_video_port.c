...
OSCL_EXPORT_REF OMX_ERRORTYPE base_video_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
    ...
    if (!(*openmaxStandPort)) {
        *openmaxStandPort = malloc(sizeof (omx_base_video_PortType));
     /** @@ Modified code
     * added checking validation of a pointer variable once free
     **/
        if (!(*openmaxStandPort)) {
        ...
        memset(*openmaxStandPort, 0x00, sizeof(omx_base_video_PortType));
    }
    ...
    omx_base_video_Port->sPortParam.format.video.cMIMEType = "raw/video";
