...
OMX_ERRORTYPE BOSA_ST_InitComponentLoader(BOSA_COMPONENTLOADER *loader) {
    FILE* omxregistryfp = NULL;
    char* line = NULL;
    char *libname = NULL;
    int num_of_comp=0;
    stLoaderComponentType** templateList = NULL;
    stLoaderComponentType** stComponentsTemp = NULL;
    void* handle;
    ...
    if (libname == NULL) {
        ...
        err = ENOMEM;
        goto ERROR_BOSA_ST_InitComponentLoader;
    }
        ...
        *(line+index_readline) = '\0';
        ...
        if((handle = dlopen(libname, RTLD_NOW)) == NULL) {
            ...
            if ((fptr = dlsym(handle, "omx_component_library_Setup")) == NULL) {
                ...
                stComponentsTemp = malloc(num_of_comp*sizeof(stLoaderComponentType*));
                memset(stComponentsTemp, 0x00, sizeof(stLoaderComponentType*));
                for (i = 0; i<num_of_comp; i++) {
                    stComponentsTemp[i] = malloc(sizeof(stLoaderComponentType));
                    if (!stComponentsTemp) {
                        DEBUG(DEB_LEV_ERR, "Insufficient memory in %s\n", __func__);
                        err = ENOMEM;
                        goto ERROR_BOSA_ST_InitComponentLoader;
                    }
                    memset(stComponentsTemp[i], 0x00, sizeof(stLoaderComponentType));
                }
...
ERROR_BOSA_ST_InitComponentLoader:
